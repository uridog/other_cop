...
socket_running = True
client4 = Client(s)
start_window = Ui_startWindow(client4)
game_window = Ui_gameWindow(client4)
round_num = 0
try:
...
while client4.screen_state == -1:
    pass
if client4.screen_state == 0:
    sign_in_window = Ui_signUpWindow(client4)
    try:
        print("before")
        ...
        pass
    while client4.username == "":
        pass
    s.send(("username:" + client4.username).encode())
    current_username = client4.username
    data = s.recv(1024).decode()
    print(data)
    while "taken" in data:
        sign_in_window.username_taken.show()
        while client4.username == current_username:
            pass
        s.send(("username:" + client4.username).encode())
        data = s.recv(1024).decode()
    userInfo = client4.username + " " + client4.first_name + " " + client4.last_name + " " + client4.password
    client4.client_singed = True
    s.send(userInfo.encode())
    data = s.recv(1024).decode()
    print(" fgbfgb" + data)
elif client4.screen_state == 1:
    log_in_window = Ui_LogInWindow(client4)
    try:
    ...
    while client4.username == "":
        pass
    s.send(("usernameold:" + client4.username).encode())
    old_username = client4.username
    data = s.recv(1024).decode()

    while "found." in data:
        log_in_window.label_2.show()
        while client4.username == old_username:
            pass
        s.send(("username:" + client4.username).encode())
        old_username = client4.username
        data = s.recv(1024).decode()
    s.send(("password:" + client4.password).encode())
    old_password = client4.password
    data = s.recv(1024).decode()
    print("asdds" + data)
    while "wrong:" in data:
        log_in_window.label_2.show()
        while client4.password == old_password:
            pass
        old_password = client4.password
        s.send(("password:" + client4.password).encode())
        data = s.recv(1024).decode()
...
while socket_running:
    client4.client_ready = False
    round_num = 0
    client4.screen_state = 2
    ready_window = Ui_readyWindow(client4)
    client4.game_start = False
    try:
        w = th.Thread(target=show_ready_window, args=(ready_window,))
        w.start()
    except:
        pass
    while client4.client_ready is False:
        pass
    s.send("ready: indeed".encode())
    client4.screen_state = 3
    try:
